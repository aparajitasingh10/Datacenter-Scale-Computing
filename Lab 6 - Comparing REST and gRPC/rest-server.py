...
@app.route('/api/add/<X>/<Y>', methods=['POST', 'GET'])
def add(X,Y):
    r_add = request
    ...
    response_pickled_add = jsonpickle.encode(response_add)
    
    return Response(response = response_pickled_add, status=200, mimetype="application/json")
